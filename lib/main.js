...
Pusher.prototype._init = function() {
  ...
  // Check if all necessary configuraiton attributes are present
  if(!this['pusher-appId'] || !this['pusher-key'] || !this['pusher-secret']) {
    throw new Error('Configuration keys `pusher-appId`, `pusher-key`, `pusher-secret` need to be present!');
  }

  this.pusher = new PusherLib({
    appId: this['pusher-appId'],
    key: this['pusher-key'],
    secret: this['pusher-secret']
  });
}
...
Pusher.prototype.trigger = function(channel, event, data, socketId, callback) {
  this.pusher.trigger(channel, event, data, socketId, function(err, req, res) {
    callback.call(null, err, res.statusCode);
  });
}